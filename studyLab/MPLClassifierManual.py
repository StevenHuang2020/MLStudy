"""
Steven 18/03/2020
MLPClassifier manual operation by epochs
"""
...
def trainWarmStart(x_train, y_train, x_test, y_test):
    clf = MLPClassifier(hidden_layer_sizes=(50,), max_iter=1, learning_rate_init=.1,warm_start=True)
    iter = 50
    scores_train = []
    scores_test = []
    ...
    for i in range(iter):
        ...
        scores_train.append(clf.score(x_train, y_train)) # SCORE TRAIN
        scores_test.append(clf.score(x_test, y_test))  # SCORE TEST
    
        lossTrain = mean_squared_error(clf.predict(x_train),y_train)
        lossTest = mean_squared_error(clf.predict(x_test),y_test)
        lossTestHamming = hamming_loss(clf.predict(x_test),y_test)
        #lossTestHinge = hinge_loss(clf.predict(x_test),y_test)
        print(i,lossTrain,lossTest,lossTestHamming)
    """ Plot """
...
def train(x_train, y_train, x_test, y_test):
    ...
    while epoch < N_EPOCHS:
        ...
        #accTest2 = accuracy_score(y_test, mlp.predict(x_test))
        #print(accTest,accTest2)
    ...
    #plotResultError(scores_train, scores_test, mlp.loss_curve_)
    pass
...
def main():
    ...
    #MLPClassifierModel(x_train, x_test, y_train, y_test)
    trainWarmStart(x_train, y_train, x_test, y_test)
